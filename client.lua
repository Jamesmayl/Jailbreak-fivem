      ...
      while jailbreak do 
        Wait(1)
          ...
          if #(vector3(1643.5603027344,2585.4670410156,45.564853668213) - GetEntityCoords(PlayerPedId())) < 1 then
            exports["framework-base"]:getModule("Util"):MissionText("Enter Court Yard", 500)
          ...
          elseif #(vector3(1636.2059326172,2565.4235839844,45.56485748291) - GetEntityCoords(PlayerPedId())) < 1 then
            exports["framework-base"]:getModule("Util"):MissionText("Exit Court Yard", 500)
            if not teleported then
              teleported = true
...
function RoleplayStats()
    ...
    if exports["framework-inventory"]:hasEnoughOfItem("shitlockpick",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["framework-inventory"]:hasEnoughOfItem("jailfood",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["framework-inventory"]:hasEnoughOfItem("methbag",1,false) then
        totalroleplay = totalroleplay + 15
    end

    if exports["framework-inventory"]:hasEnoughOfItem("assphone",1,false) then
        totalroleplay = totalroleplay + 15
    ...
    if exports["framework-inventory"]:hasEnoughOfItem("slushy",1,false) then
        totalroleplay = totalroleplay + 10
    end
...
function InmateHasAll()
    if exports["framework-inventory"]:hasEnoughOfItem("slushy",1,false)
        and
        exports["framework-inventory"]:hasEnoughOfItem("-1810795771",1,false)
        and
        exports["framework-inventory"]:hasEnoughOfItem("methbag",1,false)
        and
        exports["framework-inventory"]:hasEnoughOfItem("assphone",1,false)
        and
        exports["framework-inventory"]:hasEnoughOfItem("slushy",1,false)
    then
        return true
    else
        return false
...
AddEventHandler('swappingCharsLoop', function()
    ...
    if SwappingCharacters and not outofrange then
        ...
        DoScreenFadeOut(500)
        Citizen.Wait(1000)
        TriggerEvent("framework-base:clearStates")
        exports["framework-base"]:getModule("SpawnManager"):Initialize()
        relogging = true
        imjailed = false
