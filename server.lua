...
AddEventHandler("updateJailTime", function(minutes)
    local src = source
    local user = exports["framework-base"]:getModule("Player"):GetUser(src)
...
RegisterCommand('unjail', function(source, args)
    ...
    local user = exports["framework-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["framework-base"]:getModule("Player"):GetUser(args[1])

    if user:getVar("job") == 'police' or user:getVar("job") == 'doc' then
        if args[1] and exports["framework-base"]:getModule("Player"):GetUser(tonumber(args[1])) then
